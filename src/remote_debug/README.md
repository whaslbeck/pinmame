...
- **Advanced Debugging**:
  - Real-time Disassembler for M6809 and ADSP CPUs.
...
- `GET /api/debugger/dasm?addr=[HEX]&lines=[INT]&cpu=[ID]`: JSON disassembly starting at address.
- `GET /api/debugger/messages`: JSON list of debugger console messages (BP hits, memory logs).
...
- `GET /api/debugger/breakpoints?cmd=add&addr=[HEX]`: Add a new instruction breakpoint.
