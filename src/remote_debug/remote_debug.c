...
typedef struct { UINT32 adr; int mode; int enabled; } watchpoint_t;
static watchpoint_t watchpoints[128];
static int watchpoint_count = 0;

/* --- Logs & Messages --- */
typedef struct { UINT32 adr; int length; int write; UINT32 pc; int cpunum; } instr_log_t;
...
void remote_debug_add_message(const char *msg) {
    strncpy(msg_queue[msg_head], msg, 127); msg_queue[msg_head][127] = 0;
    msg_head = (msg_head + 1) % MSG_QUEUE_SIZE; if (msg_count < MSG_QUEUE_SIZE) msg_count++;
}

void remote_debug_init(void) {
    pthread_mutexattr_t attr; pthread_mutexattr_init(&attr);
    pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE);
    pthread_mutex_init(&mame_mutex, &attr); pthread_mutexattr_destroy(&attr);
    if (pmoptions.start_paused) { is_paused = 1; printf("Remote Debugger: Paused on start\n"); }
    should_quit = 0; breakpoint_count = 0; watchpoint_count = 0; msg_count = 0; log_count = 0;
    remote_debug_ready = 1; http_server_start(pmoptions.http_port, api_handler);
}

void remote_debug_exit(void) { remote_debug_ready = 0; http_server_stop(); pthread_mutex_destroy(&mame_mutex); }

int remote_debug_is_paused(void) {
    if (should_quit) return 0;
    if (step_requested) { step_requested = 0; return 0; }
    if (is_paused) usleep(10000);
    return is_paused;
}

void remote_debug_set_paused(int paused) { is_paused = paused; }
void remote_debug_step(void) { is_paused = 1; step_requested = 1; }
void remote_debug_quit(void) { printf("Remote Debugger: Quit\n"); should_quit = 1; }
...
void remote_debug_watchpoint_add(UINT32 adr, int mode) {
    remote_debug_lock(); if (watchpoint_count < 128) { watchpoints[watchpoint_count].adr = adr; watchpoints[watchpoint_count].mode = mode; watchpoints[watchpoint_count].enabled = 1; watchpoint_count++; char b[128]; sprintf(b, "WP added at %04X", adr); remote_debug_add_message(b); } remote_debug_unlock();
}
void remote_debug_watchpoint_clear(void) { remote_debug_lock(); watchpoint_count = 0; remote_debug_add_message("Watchpoints cleared"); remote_debug_unlock(); }
...
void remote_debug_memref(UINT32 adr, int length, int write) {
    int active_cpu = cpu_getactivecpu(); if (active_cpu < 0) return;
    for (int i = 0; i < watchpoint_count; i++) {
    ...
    int logged = 0; for (int i = 0; i < instr_addr_count; i++) { if (adr >= instr_addrs[i] && adr < instr_addr_count + length) { logged = 1; break; } }
    if (logged) {
        remote_debug_lock(); instr_log_t *e = &instr_log[log_head]; e->adr = adr; e->length = length; e->write = write; e->cpunum = active_cpu; e->pc = activecpu_get_reg(REG_PC);
        log_head = (log_head + 1) % LOG_SIZE; if (log_count < LOG_SIZE) log_count++; remote_debug_unlock();
    }
}
...
void remote_debug_get_log(char **buffer, int *len) {
    remote_debug_lock(); *buffer = malloc(log_count * 128 + 256); char *p = *buffer; p += sprintf(p, "{\"logs\": [");
    for (int i = 0; i < log_count; i++) { int idx = (log_head - log_count + i + LOG_SIZE) % LOG_SIZE; instr_log_t *e = &instr_log[idx]; if (i > 0) p += sprintf(p, ","); p += sprintf(p, "{\"cpu\": %d, \"pc\": %u, \"adr\": %u, \"len\": %d, \"write\": %d}", e->cpunum, e->pc, e->adr, e->length, e->write); }
    p += sprintf(p, "]}"); *len = p - *buffer; remote_debug_unlock();
}

void remote_debug_instr_add(UINT32 adr) { remote_debug_lock(); if (instr_addr_count < 128) instr_addrs[instr_addr_count++] = adr; remote_debug_unlock(); }
void remote_debug_instr_clear(void) { remote_debug_lock(); instr_addr_count = 0; log_head = 0; log_count = 0; remote_debug_unlock(); }

/* Visuals */
static struct mame_display *last_display = NULL;
void remote_debug_frame_update(struct mame_display *display) { remote_debug_lock(); last_display = display; remote_debug_unlock(); }
void write_ppm_to_buffer(struct mame_bitmap *bitmap, char **buffer, int *len) {
    if (!bitmap || !last_display) return; int width = bitmap->width; int height = bitmap->height; if (width > 2048 || height > 2048) return;
    *len = width * height * 3 + 1024; *buffer = malloc(*len); char *p = *buffer; p += sprintf(p, "P6\n%d %d\n255\n", width, height);
